"""
Brandnew 모델 테스트용 웹캠 동작 인식

사용법:
    python webcam_test_brandnew.py
    python webcam_test_brandnew.py --rhythm  # 리듬 모드 (100BPM, 8회 반복)
    python webcam_test_brandnew.py --url https://heungbuja.site/motion/api/ai/brandnew/analyze  # 배포 서버

배포 서버: https://heungbuja.site/motion/api/ai/brandnew/analyze
로컬 서버: http://localhost:8000/api/ai/brandnew/analyze
...
def send_to_api(frames, action_code, action_name, api_url):
    ...
    server_type = "배포 서버" if is_https else "로컬 서버"

    print(f"\n🔍 AI 분석 중 ({server_type})...", end="", flush=True)
    ...
    try:
        response = requests.post(
            ...
            timeout=60,
            verify=is_https,  # HTTPS일 때만 인증서 검증
        )
...
def main():
    ...
    parser.add_argument(
        "--url",
        default="http://localhost:8000/api/ai/brandnew/analyze",
        help="Brandnew Motion API URL (기본값: 로컬 서버)"
    )
    ...
    server_type = "배포 서버" if api_url.startswith("https://") else "로컬 서버"

    print("\n" + "=" * 60)
    print("🆕 Brandnew 모델 웹캠 동작 테스트")
    print("=" * 60)
    print(f"🌐 API: {api_url} ({server_type})")
    ...
    try:
        while True:
            ...
            # 간단한 UI
            status_text = f"🆕 Brandnew: {action_name} ({action_code}) [{server_type}]"
            if rhythm_enabled:
                status_text += f" | 🎵 {args.bpm}BPM x{args.reps}"
            ...
            cv2.imshow("Brandnew 모델 웹캠 테스트", frame)
